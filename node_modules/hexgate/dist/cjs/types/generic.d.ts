 ...
 * ```
 */
export type SplitDashesAndSlashes<T extends string> = Flat<Split<T, '-'> extends infer S extends string[] ? {
    [K in keyof S]: Split<Assert<S[K], string>, '/'>;
} : never> extends infer F extends string[] ? F : never;
/** Boolean - true if `A` extends `B` extends `A` */
export type Exact<A, B> = Extract<A, B> extends never ? false : Extract<B, A> extends never ? false : true;
 ...
 */
export type ExtractDefined<T> = Pick<T, {
